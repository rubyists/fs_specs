...
class VmListener < FSL::Inbound
  ...
  def post_init
    ...
    orig = @sock1.originate(target_options: {origination_caller_id_number: @spec_id}, target: "sofia/external/4000@#{@server2}", endpoint: '&park')
...
if __FILE__ == $0
  ...
  warn "Starting VmListener, someone better call voicemail quick!"
  EM.run do
    EM.add_periodic_timer(20) { |e| EM.stop }
    EM.connect(@server2, 8021, VmListener, @sock1, @sock2, @server1, @server2, "1000", "1001")
  end
  #p VmListener::PLAYBACK_FILES
end
